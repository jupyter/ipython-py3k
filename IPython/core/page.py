...
def page_dumb(strng, start=0, screen_lines=25):
    ...
    if len(screens) == 1:
        print(os.linesep.join(screens[0]), file=Term.cout)
    else:
        last_escape = ""
        for scr in screens[0:-1]:
            hunk = os.linesep.join(scr)
            print(last_escape + hunk, file=Term.cout)
            if not page_more():
                return
            esc_list = esc_re.findall(hunk)
            if len(esc_list) > 0:
                last_escape = esc_list[-1]
        print(last_escape + os.linesep.join(screens[-1]), file=Term.cout)
...
def page(strng, start=0, screen_lines=0, pager_cmd=None):
    ...
    if numlines <= screen_lines :
        ...
        print(str_toprint, file=Term.cout)
    else:
