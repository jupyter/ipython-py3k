...
class Hub(LoggingFactory):
    ...
    def save_queue_request(self, idents, msg):
        ...
        if eid is None:
            ...
            self.log.debug("queue::    valid are: %s"%(list(self.by_ident.keys())))
            return
    ...
    def register_engine(self, reg, msg):
        ...
        if queue in self.by_ident:
            ...
            for h, pack in self.incoming_registrations.items():
                if heart == h:
                    try:
                        raise KeyError("heart_id %r in use"%heart)
                    except:
    ...
    def resubmit_task(self, client_id, msg):
        ...
        invalid_ids = [m for m in found_ids if m in self.pending]
        ...
        rec = empty_record()
        list(map(rec.pop, ['msg_id', 'header', 'content', 'buffers', 'submitted']))
    ...
    def _extract_record(self, rec):
        ...
        if rec['result_buffers']:
            buffers = list(map(str, rec['result_buffers']))
        else:
            buffers = []
