...
def serialize_object(obj, threshold=64e-6):
    ...
    if isinstance(obj, (list, tuple)):
        ...
        slist = list(map(serialize, clist))
        for s in slist:
...
def unserialize_object(bufs):
    ...
    if isinstance(sobj, (list, tuple)):
        for s in sobj:
            if s.data is None:
                s.data = bufs.pop(0)
        return uncanSequence(list(map(unserialize, sobj))), bufs
    elif isinstance(sobj, dict):
        newobj = {}
        for k in sorted(sobj.keys()):
            s = sobj[k]
            if s.data is None:
                s.data = bufs.pop(0)
...
def unpack_apply_message(bufs, g=None, copy=True):
    ...
    args = uncanSequence(list(map(unserialize, sargs)), g)
    kwargs = {}
    for k in sorted(skwargs.keys()):
        sa = skwargs[k]
        if sa.data is None:
            m = bufs.pop(0)
            if sa.getTypeDescriptor() in ('buffer', 'ndarray'):
...
def _pull(keys):
    ...
    if isinstance(keys, (list,tuple, set)):
        ...
        return list(map(user_ns.get, keys))
    else:
