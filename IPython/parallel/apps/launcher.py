...
class WindowsHPCControllerLauncher(WindowsHPCLauncher):
    ...
    job_file_name = CUnicode('ipcontroller_job.xml', config=True)
...
class WindowsHPCEngineSetLauncher(WindowsHPCLauncher):
    ...
    job_file_name = CUnicode('ipengineset_job.xml', config=True)
    extra_args = List([], config=False)
...
class BatchSystemLauncher(BaseLauncher):
    ...
    batch_template_file = CUnicode('', config=True)
    # The filename of the instantiated batch script.
    batch_file_name = CUnicode('batch_script', config=True)
    # The PBS Queue
    queue = CUnicode('', config=True)
...
class PBSControllerLauncher(PBSLauncher):
    ...
    batch_file_name = CUnicode('pbs_controller', config=True)
...
class PBSEngineSetLauncher(PBSLauncher):
    ...
    batch_file_name = CUnicode('pbs_engines', config=True)
    default_template= CUnicode("""#!/bin/sh
#PBS -V
#PBS -N ipengine
...
class SGEControllerLauncher(SGELauncher):
    ...
    batch_file_name = CUnicode('sge_controller', config=True)
...
class SGEEngineSetLauncher(SGELauncher):
    ...
    batch_file_name = CUnicode('sge_engines', config=True)
    default_template = CUnicode("""#$$ -V
#$$ -S /bin/sh
#$$ -N ipengine
%s --cluster-dir $cluster_dir
